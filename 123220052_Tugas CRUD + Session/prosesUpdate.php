...
$query = mysqli_query($connect, "UPDATE menu SET namaMenu='$namaMenu', category='$kategori', price='$harga' WHERE
id_menu='$id'") or die(mysqli_error($connect));
    ?>
    <center>
        ...
        if ($query) {
            if (mysqli_affected_rows($connect) > 0) {
                echo "<p>Proses update berhasil, ingin lihat hasil <a href='main.php'><button class=\"btn\" style=\"background-color: #465771; color: #ffff\" type=\"submit\">disini</button></a></p>";
            } else {
                echo "<p>ID menu tidak ditemukan atau tidak ada perubahan yang terjadi</p>";
            }
        } else {
            echo "<p?>Proses update gagal</p>"; } mysqli_close($connect); ?>
                        </div>
                        <hr>
                        <div class="row">
                            <div class="col">
